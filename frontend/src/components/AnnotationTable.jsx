...
import { Card, Table, Space, Button, Tag, Tooltip } from "antd";
...
export default function AnnotationTable({
  ...
  const columns = [
    ...
    { 
      ...
      width: 120,
      render: (status, r) => (
         <Tag color={r._isSelection ? "warning" : "blue"} bordered={false} className="rounded-full px-3">
            {status || "无状态"}
         </Tag>
      )
    },
    { 
      ...
      dataIndex: "content",
      render: (text, r) => (
        <span className={`line-clamp-2 ${r._isSelection ? "text-gray-400 italic" : "text-gray-700"}`}>
          {text || "-"}
        </span>
      )
    },
    ...
    },
    {
      ...
      render: (_, r) => (
        <Space size="small" onClick={(e) => e.stopPropagation()}>
          {r._isSelection ? (
            <>
              <Button size="small" type="primary" icon={<PlusOutlined />} onClick={() => onCreateFromSelection(r)}>
                添加
              </Button>
              <Button size="small" type="text" danger icon={<CloseOutlined />} onClick={() => onRemoveSelection(r)}>
                丢弃
              </Button>
            </>
          ) : (
  ...
  return (
    <Card 
      title={<span className="font-bold text-gray-800">标注列表 <span className="text-gray-400 text-sm font-normal ml-2">{rows.length} 项</span></span>} 
